...
import { ICreatePostRequest } from 'src/app/core/interfaces/ICreatePostRequest';
import { Category, IPost } from 'src/app/core/interfaces/INews';
...
import { IImageValidationConfig, IHtmlEditorStats, IValidationError } from 'src/app/shared/interfaces/Ihtml-editor.interface';
...
export class EditPostComponent implements OnInit {
          ...
          if (res.isSuccess) {
            ...
            if (this.postDetails) {
              this.postForm.patchValue({
                ...
                category: this.postDetails.categoryId ?? '',
                coverImage:  null
              });
              this.coverImagePreview = this.postDetails.coverImage ?? '';
            }
  ...
  // }
   ...
   async publishPost(postStatus?:string): Promise<void> {
    try {
      ...
      if (this.postId) {
         this.newsService.updatePost(postData,this.postId).subscribe({
        next: (response) => {
